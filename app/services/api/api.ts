...
export const createAPI = (config: ApiConfig = DEFAULT_API_CONFIG) => {
	...
	const getCategories = async (): Promise<Types.GetCategoriesResult> => {
		const response: ApiResponse<any> = await api.get('/categories');
		...
		return { kind: 'ok', categories: response.data };
	};
